  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div className="bg-[#faf6f8] rounded-2xl p-5 border border-[#f0e8ee] flex flex-col items-center text-center">
                    ...
                    <span className="text-2xl font-bold text-[#3f2b4d]">
                      {prediction?.average_cycle_length ? `${Math.round(prediction.average_cycle_length)} Days` : "-"}
                    </span>
                  </div>
                  ...
                  <div className="bg-[#fff0f6] rounded-2xl p-5 border border-[#f2d6e4] flex flex-col items-center text-center">
                    ...
                    <span className="text-2xl font-bold text-[#3f2b4d]">
                      {prediction?.average_period_length ? `${Math.round(prediction.average_period_length)} Days` : "-"}
                    </span>
                  </div>
                  ...
                  <div className="bg-[#f0f9ff] rounded-2xl p-5 border border-[#e0f2fe] flex flex-col items-center text-center">
                    ...
                    <span className="text-xl font-bold text-[#3f2b4d] mt-1">
                      {cycles.length >= 3 ? "Regular" : "Learning..."}
                    </span>
                  </div>
                </div>
      ...
      {editModalOpen && (
        <div className="modal-overlay" onClick={() => setEditModalOpen(false)}>
          <div className="modal-content max-w-sm w-full" onClick={e => e.stopPropagation()}>
            <h2 className="text-2xl font-bold mb-6">{editing ? "Edit cycle" : "Log cycle"}</h2>
            <form onSubmit={saveCycle} className="space-y-5">
              <div>
                ...
                <DatePicker
                  ...
                  className="input w-full p-3 rounded-xl border border-gray-200"
                  required
                />
              </div>
              <div>
              ...
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={() => setEditModalOpen(false)} className="btn-outline flex-1 py-3 rounded-xl">
                  Cancel
                </button>
                <button type="submit" disabled={saving} className="btn-primary flex-1 py-3 rounded-xl">
                  {saving ? "Saving..." : "Save"}
                </button>
              </div>
            </form>
        ...
        <div className="modal-overlay" onClick={() => setDeleteModalOpen(false)}>
          <div className="modal-content max-w-sm w-full" onClick={e => e.stopPropagation()}>
            <div className="text-center">
              ...
              <h2 className="text-xl font-bold mb-2">Delete Cycle?</h2>
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteModalOpen(false)} className="btn-outline flex-1 py-2.5 rounded-xl font-medium">
                Cancel
              </button>
              <button onClick={executeDelete} className="bg-red-500 hover:bg-red-600 text-white flex-1 py-2.5 rounded-xl font-medium transition-colors">
                Delete
              </button>
            </div>
          </div>
      ...
      {calendarActionOpen && selectedCalendarDate && (
        <div className="modal-overlay" onClick={() => setCalendarActionOpen(false)}>
          <div className="bg-white p-6 rounded-[2rem] shadow-xl max-w-sm w-full relative overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <button
                className="w-full flex items-center gap-3 p-4 bg-[#fff0f6] hover:bg-[#ffe6f0] text-[#b06a94] rounded-2xl font-semibold transition-colors"
                onClick={async () => {
                  setCalendarActionOpen(false)
                  await quickLogPeriodForDate(selectedCalendarDate)
                }}
              >
                <div className="bg-white p-2 rounded-full shadow-sm"><Droplet size={18} className="text-[#ff7eb6]" /></div>
                Quick log period
              </button>

              <button
                className="w-full flex items-center gap-3 p-4 bg-[#f0f9ff] hover:bg-[#e0f2fe] text-[#0369a1] rounded-2xl font-semibold transition-colors"
                onClick={() => {
                  setCalendarActionOpen(false)
                  setDailyLogOpen(true)
                }}
              >
                <div className="bg-white p-2 rounded-full shadow-sm"><Pencil size={18} className="text-[#38bdf8]" /></div>
                Edit daily log
              </button>
            </div>
            ...
            <button
              ...
              className="w-full mt-6 py-3 text-sm font-semibold text-[#7d6b86] hover:text-[#3f2b4d] transition-colors"
            >
              Cancel
            </button>
          </div>
